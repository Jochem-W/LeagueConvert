...
public static class OBJConverter
{
    ...
    {
        foreach (var model in wgeo.Models)
        {
    ...
    public static IEnumerable<Tuple<string, OBJFile>> ConvertMGEOModels(MapGeometry mgeo)
    {
        foreach (var model in mgeo.Models)
        {
            ...
            foreach (var vertex in model.Vertices)
            {
                ...
                if (vertex.DiffuseUV != null) uvs.Add(vertex.DiffuseUV.Value);
            }

            yield return new Tuple<string, OBJFile>(model.Name,
                new OBJFile(vertices, model.Indices.Select(x => (uint) x).ToList(), uvs, normals));
        }
    }
